...
Docked servers are shared by all agents — the daemon runs one set of server processes and isolates agents by policy, not by per-agent processes.

<CardGroup cols={2}>
  <Card title="Stdio servers" icon="box" color="#10b981">
    Started and managed by the daemon as shared docked server processes.
  </Card>
