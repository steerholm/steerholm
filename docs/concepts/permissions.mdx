...
harbour grant agent filesystem --tool "read_text_file" --args "path=/home/user/projects/**"

# Regex: allow only SELECT queries
harbour grant agent database --tool "query" --args "sql=re:^SELECT\s.*"

# Multiple argument policies on one tool
harbour grant agent database --tool "query" --args "sql=re:^SELECT\s.*" "db=production"
```
...
<Warning>
  The client does not declare which agent it is. The daemon resolves the agent from the Bearer token. The client cannot influence how it is identified.
</Warning>
