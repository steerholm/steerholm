...
MCP Harbour uses a **whitelist-based** permission model. An agent with no policy gets zero tools — the harbour creates an empty policy that denies everything.
...
<Steps>
  <Step title="Tool lookup">
    The harbour resolves which server owns the requested tool. If the tool doesn't exist on any server the agent has access to → `AUTHORIZATION_DENIED`.
  </Step>
...
Agents are created with `harbour add agent <name>`. Each gets:
...
<Warning>
  The caller does not declare which agent it is. Harbour resolves the agent from the access key, so the caller cannot influence how it is identified.
</Warning>
