...
MCP Harbour is one surface with two faces. To your **agents** it's an **action
...
```bash
harbour add server filesystem --command "..."   # register a capability
harbour add agent coding-agent                  # register an agent
harbour grant coding-agent filesystem --tool "read_*" --args "path=/home/**"
harbour show agent coding-agent                 # what can it reach?
harbour revoke coding-agent filesystem          # take access away
```
...
through Harbour**. The agent never talks to the server directly — it connects to
Harbour, and Harbour forwards each call to the server *only where the grant and
its policy allow*. The grant is the edge; Harbour is what the traffic flows
...
1. `harbour add agent <name>` registers the agent and issues its access key.
2. You configure the tool with that key so it opens its connection to Harbour —
   see [Connect an agent](/guides/connect-an-agent).
3. `harbour grant ...` gives the agent access to the servers it should reach.

<Note>
  Step 2 is the current **manual** last mile. It's part of *adding an agent* —
  just not yet automated. We're working toward Harbour setting up that connection
