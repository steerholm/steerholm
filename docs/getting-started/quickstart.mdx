...
Once it starts working, see what it actually did:

```bash
holm log             # every call, and whether it was allowed
holm log --follow    # watch them as they happen
```
...
Both outcomes are recorded. `holm log --status denied` shows the refusal above,
with the reason that produced it — see the [audit log](/reference/audit-log).
