...
Harbour is the control plane your agents operate in. Agents themselves connect to
...
to authenticate — Harbour derives the agent from the key, so agents cannot
...
argument constraints), and how to connect a client with its key.
...
key stops working immediately — update the agent's client config with the new
key.
