...
| `--server` | — | Only calls to this server |
...
<Note>
  An agent name can be reused by removing and re-adding an agent, so a name
  filter can span two different principals. `--agent` also accepts the immutable
  agent id (`holm show agent` prints it), which isolates one of them; `holm log`
  says so when a name covers more than one.
</Note>
