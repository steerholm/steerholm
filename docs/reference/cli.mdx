...
Remove a server from Steerholm, along with every agent's grants on it.

```bash
holm remove server git
```

Grants go with the server, the same way removing an agent takes its policy. A
name can be reused, so a grant that outlived its server would silently apply to
whatever is added under that name next. The command reports which agents lost
grants.

<Note>
  To change a server's command, URL, or environment **without** losing its
  grants, use [`holm modify server`](#holm-modify-server) instead of removing and
  re-adding it.
</Note>
...
<Note>
  **Names** must be letters, digits, `_`, `-`, or `.`, and must start with a
  letter or `_`. `agt_` and `srv_` are reserved prefixes — ids use them, and the
  CLI accepts either a name or an id in places like `holm log --agent`, so a
  name must never be confusable with one.
</Note>
...
Remove an agent, its access key, and its policy — which takes every grant it
held. The key is revoked immediately.
...
The server must already be added — a grant records the agent's and server's
**ids**, not their names, so there is nothing to point at otherwise. That is also
why a grant survives `holm rotate` and `holm modify server` but not
`holm remove`.
